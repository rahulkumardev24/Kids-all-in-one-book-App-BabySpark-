...
class DivisionController extends GetxController {
  final divisionProblems = <Map<String, dynamic>>[].obs;
  ...
  void onInit() {
    ...
    final correctAnswer = divisionProblems[currentProblemIndex.value]['result'].toString();
  ...
  void generateRandomProblems() {
    ...
    for (int i = 0; i < 15; i++) {
      ...
      if (i < 5) {
        // Easy problems: Division by 2
        result = 1 + random.nextInt(4); // 1-4
        num2 = 2; // Always divide by 2
        num1 = result * num2; // Calculate dividend
      } else if (i < 10) {
        // Medium problems: Division by 2, 3, 4
        result = 2 + random.nextInt(4); // 2-5
        num2 = 2 + random.nextInt(3); // 2-4
        num1 = result * num2;
      } else {
        // Hard problems: Division by numbers up to 5
        result = 3 + random.nextInt(4); // 3-6
        num2 = 2 + random.nextInt(4); // 2-5
        num1 = result * num2;
      }

      divisionProblems.add({
        'num1': num1,
        'num2': num2,
        'result': result,
        'remainder': 0 // Simple division without remainders for now
      });
    }
  }
  ...
  void checkAnswer(String selectedOption) {
    ...
    final problem = divisionProblems[currentProblemIndex.value];
    ...
    if (selectedOption == correctAnswer) {
      ...
      Future.delayed(const Duration(seconds: 1), () {
        if (currentProblemIndex.value < divisionProblems.length - 1) {
          goToNextProblem();
        } else {
  ...
  List<String> generateOptions(String correctAnswer) {
    ...
    while (options.length < 4) {
      int randomOption;
      do {
        // For division, results are typically smaller numbers
        randomOption = 1 + random.nextInt(8);
      } while (options.contains(randomOption.toString()) || randomOption == correct);
  ...
  void goToNextProblem() {
    ...
    if (currentProblemIndex.value < divisionProblems.length - 1) {
      ...
      // Trigger completion callback when it's the last problem
      if (onCompletion != null) {
        onCompletion!();
      }
    }
  ...
  Future<void> speakCurrentProblem(int session) async {
    ...
    final line = "$num1 divided by $num2 equals $result";
