...
class MultiplicationController extends GetxController {
  final multiplicationProblem = <Map<String, dynamic>>[].obs;
  ...
  void onInit() {
    ...
    final correctAnswer =
        multiplicationProblem[currentProblemIndex.value]['result'].toString();
  ...
  void generateRandomProblems() {
    multiplicationProblem.clear();

    for (int i = 0; i < 15; i++) {
      int num1, num2;

      if (i < 5) {
        num1 = 1 + random.nextInt(2);
        num2 = 1 + random.nextInt(2);
      } else {
        num1 = 2 + random.nextInt(2);
        num2 = 2 + random.nextInt(2);
      }
  ...
  void checkAnswer(String selectedOption) {
    ...
    final problem = multiplicationProblem[currentProblemIndex.value];
    ...
    if (selectedOption == correctAnswer) {
      ...
      Future.delayed(const Duration(seconds: 1), () {
        if (currentProblemIndex.value < multiplicationProblem.length - 1) {
          goToNextProblem();
        } else {
  ...
  Future<void> speakCurrentProblem(int session) async {
    ...
    final line = "$num1 multiplied by $num2 equals $result";
